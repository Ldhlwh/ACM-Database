...
public class BTreeInternalPage extends BTreePage {
	 ...
	 * @param tid
	 * @param dirtypages
	 * @param cur current pageNumber, for output
	 */
	public void print(TransactionId tid, HashMap<PageId, Page> dirtypages, int cur)
	{
		System.err.printf("%d -> ", cur);
		Iterator<BTreeEntry> it = iterator();
		BTreeEntry entry = null;
		while(it.hasNext())
		{
			entry = it.next();
			System.err.printf("%d [%s] ", entry.getLeftChild().pageNumber(), entry.getKey().toString());
		}
		...
		while(it.hasNext())
		{
			...
			try
			{
				BTreePage p = (BTreePage)((BTreeFile)Database.getCatalog().getDatabaseFile(entry.getLeftChild().getTableId())).getPage(tid, dirtypages, entry.getLeftChild(), Permissions.READ_ONLY);
				p.print(tid, dirtypages, entry.getLeftChild().pageNumber());
			}
			catch(Exception e) {}
		}
		try
		{
			BTreePage p = (BTreePage)((BTreeFile)Database.getCatalog().getDatabaseFile(entry.getRightChild().getTableId())).getPage(tid, dirtypages, entry.getRightChild(), Permissions.READ_ONLY);
			p.print(tid, dirtypages, entry.getRightChild().pageNumber());
		}
		catch(Exception e) {}
	}
}
