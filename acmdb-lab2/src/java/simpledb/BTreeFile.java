...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		{
			page = (BTreePage)getPage(tid, dirtypages, pid, Permissions.READ_ONLY);
		}
		else if(pid.pgcateg() == BTreePageId.LEAF)
		{
			page = (BTreePage)getPage(tid, dirtypages, pid, perm);
			return (BTreeLeafPage)page;
		}
		
		
		IndexPredicate fieldIP = new IndexPredicate(Op.LESS_THAN_OR_EQ, f);
		...
		{
			entry = it.next();
			IndexPredicate entryIP = new IndexPredicate(Op.LESS_THAN_OR_EQ, entry.getKey());
			if(fieldIP.equals(entryIP))
			{
				return findLeafPage(tid, dirtypages, entry.getLeftChild(), perm, f);
			}
		}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		int move = page.getNumTuples() / 2;
		Iterator<Tuple> it = page.reverseIterator();
		for(int i = 0; i < move; i++)
		{
			Tuple now = it.next();
			page.deleteTuple(now);
			newLeafPage.insertTuple(now);
		}
		it = page.reverseIterator();
		BTreeEntry newEntry = new BTreeEntry(it.next().getField(keyField), page.getId(), newLeafPage.getId());
		...
		IndexPredicate fieldIP = new IndexPredicate(Op.LESS_THAN_OR_EQ, field);
		IndexPredicate entryIP = new IndexPredicate(Op.LESS_THAN_OR_EQ, newEntry.getKey());
		if(fieldIP.equals(entryIP))
		{
			return page;
		}
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int move = (page.getNumEntries() - 1)/ 2;
		Iterator<BTreeEntry> it = page.reverseIterator();
		for(int i = 0; i < move; i++)
		{
			BTreeEntry now = it.next();
			page.deleteKeyAndRightChild(now);
			newInternalPage.insertEntry(now);
		}
		it = page.reverseIterator();
		BTreeEntry pushedEntry = it.next();
		BTreeEntry newEntry = new BTreeEntry(pushedEntry.getKey(), page.getId(), newInternalPage.getId());
	...
	public ArrayList<Page> insertTuple(TransactionId tid, Tuple t)
		...
		//((BTreePage)getPage(tid, dirtypages, getRootPtrPage(tid, dirtypages).getRootId(), null)).print(tid, dirtypages, getRootPtrPage(tid, dirtypages).getRootId().pageNumber());
			...
			BTreeInternalPage parent, BTreeEntry entry, boolean isRightSibling) throws DbException
	{
		...
		{
			target = it.next();
			sibling.deleteTuple(target);
			page.insertTuple(target);
		}
		if(isRightSibling)
			entry.setKey(target.getField(keyField));
		else
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeEntry addedEntry = new BTreeEntry(parentEntry.getKey(), leftEntry.getRightChild(), rightEntry.getLeftChild());
		page.insertEntry(addedEntry);
		
		leftIt = leftSibling.reverseIterator();
		for(int i = 0; i < move; i++)
		{
			leftEntry = leftIt.next();
			addedEntry = new BTreeEntry(leftEntry.getKey(), leftEntry.getLeftChild(), leftEntry.getRightChild());
			page.insertEntry(addedEntry);
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		rightIt = rightSibling.iterator();
		for(int i = 0; i < move; i++)
		{
			rightEntry = rightIt.next();
			addedEntry = new BTreeEntry(rightEntry.getKey(), rightEntry.getLeftChild(), rightEntry.getRightChild());
			page.insertEntry(addedEntry);
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeEntry addedEntry = new BTreeEntry(parentEntry.getKey(), leftIt.next().getRightChild(), rightIt.next().getLeftChild());
		leftPage.insertEntry(addedEntry);
